...
class SPDTransform(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        self.weight = geoopt.ManifoldParameter(torch.FloatTensor(input_size, output_size).to('cpu'), manifold=Stiefel())
...
class SPDIncreaseDim(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        self.register_buffer('eye', torch.eye(output_size, input_size).to('cpu'))
...
class SPDVectorize(nn.Module):
    ...
    def forward(self, input):
        output = input[:, self.row_idx.to(self.device), self.col_idx.to(self.device)]
        return output
...
class SPDRectifiedFunction(Function):
    ...
    def forward(ctx, input, epsilon):
        ctx.save_for_backward(input, epsilon.to("cpu"))
...
class SPDRectified(nn.Module):
    ...
    def __init__(self, epsilon=1e-4, device='cpu'):
        ...
        self.device = device
        self.register_buffer('epsilon', torch.FloatTensor([epsilon], device="cpu").to("cpu"))

    def forward(self, input):
        output = SPDRectifiedFunction.apply(input, self.epsilon.to("cpu"))
        return output
